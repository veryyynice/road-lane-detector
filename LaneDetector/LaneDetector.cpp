...
Hardcodes the 4 warp source points for the test dashcam video. These were picked by pausing the video and clicking with the mouse callback.
...
LaneDetector::LaneDetector() {
    ...
    srcPoints = {
        cv::Point2f(434, 403), // top left
        cv::Point2f(806, 410), // top right
        cv::Point2f(17, 565), // bottom left
        cv::Point2f(1273, 561), // bottom right
    };

    // flat destination rectangle
    dstPoints = {
        cv::Point2f(0, 0),
        cv::Point2f(400, 0),
        cv::Point2f(0, 600),
...
Postconditions: draws red ROI trapezoid onto inputFrame in-place, fills debugView with the annotated bird's eye image, fills debugEdges with the canny mask. Returns steering angle in degrees (positive is left curve, negative is right)
*/
...
/*
calculateCurveAngle
Finds the left and right lane lines using a sliding window search, fits a polynomial to each, then computes how much the road curves by comparing the midpoint near the car vs the midpoint farther ahead.

Preconditions: warpedEdges is a 400x600 grayscale binary edge image, debugView is a BGR image the same size
Postconditions: draws sliding windows and yellow curves onto debugView. Returns steering angle in degrees, or 0 if lanes not found
*/
double LaneDetector::calculateCurveAngle(const cv::Mat& warpedEdges, cv::Mat& debugView) {
    ...
    // left peak: search left half only
    ...
    // right peak: search right half only
    ...
    for (int window = 0; window < numWindows; window++) {
        ...
        cv::rectangle(debugView, cv::Point(winX_left_min, winY_bottom), cv::Point(winX_left_max, winY_top), cv::Scalar(0, 255, 0), 2); // green is left
        cv::rectangle(debugView, cv::Point(winX_right_min, winY_bottom), cv::Point(winX_right_max, winY_top), cv::Scalar(0, 0, 255), 2); // red is right
        ...
        // re-center the next window on the mean x of found pixels, but only if we found enough pixels
        if (leftPixCount > minPix) {
            leftCurrentX = leftPixSumX / leftPixCount;
        }

        if (rightPixCount > minPix) {
            rightCurrentX = rightPixSumX / rightPixCount;
        }
    }
    ...
    if (leftLanePts.size() > 10 && rightLanePts.size() > 10) {
        ...
        // before using fits, verify they describe a believable pair of lane lines
        // both polynomials must curve in the same direction
        double leftA = leftFit.at<double>(0, 0);
        double rightA = rightFit.at<double>(0, 0);
        bool sameDirection = (leftA * rightA) >= 0.0;

        // lane width at bottom should make sense
        ...
        bool validWidth = laneWidth > 100.0 && laneWidth < 380.0;

        bool isValidFit = sameDirection && validWidth;


        // always draw the curves so the debug view shows what was detected this frame
        // yellow is accepted by the validation check, orange is rejected
        cv::Scalar curveColor = isValidFit ? cv::Scalar(0, 255, 255) : cv::Scalar(0, 100, 255);
        ...
        // if the check fails hold the last EMA value so we don't corrupt it with a bad frame
        if (!isValidFit) {
            return emaAngle;
        }

        // steering angle: compare the lane midpoint near the car vs far ahead
        // shift is how much the midpoint moves from bottom to top - positive means curving left
        ...
        // bottom minus top gives positive for left curves, negative for right curves
        // OpenCV rotates clockwise for negative angles which is correct - right curve should turn the wheel clockwise
        double shift = bottomMidX - topMidX;
        double rawAngle = shift * 0.4;


        // confidence weighted EMA
        // alpha scales with how many lane pixels were found - more pixels means cleaner fit
        // so we trust it more and update the estimate faster
...
Uses y as the input because lane lines run vertically in the birds eye view. Fitting x = f(y) instead of y = f(x) avoids the vertical line singularity.
SVD is used instead of normal equations because SVD is stable even when the points are nearly collinear.
...
/*
applyHSVMask
Isolates white and yellow lane pixels by converting to HSV and thresholding, then runs gaussian blur and canny to get clean edges.

Preconditions: frame is a valid BGR image
Postconditions: returns a binary grayscale canny edge image the same size as frame, with only edges along white/yellow regions kept
*/

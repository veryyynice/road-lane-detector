...
public:
    ...
    */
    ...
    Postconditions: draws ROI trapezoid onto inputFrame in-place, fills debugView with the bird's eye sliding window image, fills debugEdges with the canny edge mask, and returns the steering angle in degrees
    */
...
private:
    ...
    */
    ...
    Postconditions: draws sliding windows and fitted curves onto debugView. Returns steering angle in degrees, or 0 if not enough lane pixels were found
    */
    double calculateCurveAngle(const cv::Mat& warpedEdges, cv::Mat& debugView);

    /*
    polyfit
    Least squares 2nd degree polynomial fit: x = Ay^2 + By + C. Axes are swapped on purpose because lane lines run vertically in the warped view.
    ...
    */
    ...
    bool emaInit = false; // set on first valid frame so we don't blend from 0
    const double emaMinAlpha = 0.05; // alpha floor used when barely enough pixels to fit
    const double emaMaxAlpha = 0.30; // alpha ceiling used at full confidence
